...
FNAMES = os.listdir() #Creates a list of all .edf files
for file in FNAMES:
    if ".edf" not in file:
        FNAMES.remove(file)
new_FNAMES = []
for file in FNAMES:
    if ".edf" in file: #It is weird but sometimes the first loop doesn't work so this is to make sure it does
        new_FNAMES.append(file)
FNAMES = new_FNAMES

import random
random.shuffle(FNAMES) #Randomize the .edf files for classification so the model will not get always the same sequence
...
epoch_sec = 0.25 #CHANGE EPOCH_SEC BASED ON THE RESULTS IN 13_SUBJECT_MODELS.PY
nomeschema = "all_electrodes"


##   GET PREPROCESSED DATA FROM FILE  (OR PREPROCES IT ON THE SPOT)
preprocessed_data_file = './preprocessed_dataset/dataIMM_epochsec-'+str(epoch_sec)+'_schema_'+nomeschema+'_nsub_'+str(len(FNAMES))+'.h5'
...
y = Dropout(0.25, name='dropout1')(y)
...
h = Dropout(0.25, name='dropout2')(h)
...
modelname = "model_"+nomeschema+"_BiLSTM_0_25_dropout_half_fully_connected_all_data"
# Get past models
